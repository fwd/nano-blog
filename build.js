...
fs.writeFileSync(`${dest}/index.html`, ejs.render(index_html, { articles, title, site_title, metrics, contact }), { encoding: "utf8" } )
fs.writeFileSync(`${dest}/404.html`, ejs.render(index_html, { articles, title, site_title, metrics, contact }), { encoding: "utf8" } )
...
if (blog_path && !fs.existsSync(dest + '/' + blog_path)) {
	...
	fs.writeFileSync(`${dest + '/' + blog_path}/index.html`, ejs.render(index_html, { articles, title, metrics, contact }), { encoding: "utf8" } )
}

// markdown2html
var single_html = fs.readFileSync(`./themes/${theme}/single.html`, { encoding: "utf8" })
for (var article of articles) {
	var article_html = ejs.render(single_html, { articles : articles.filter(a => a.slug !== article.slug), article, title, nano_address: article.address || nano_address, metrics, contact })
